import React from "react";
import { Link } from "react-router-dom";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";

const Industries = () => {
  const industries = [
    { name: "Military & Defence", link: "/industries/military-defence" },
    { name: "Agriculture", link: "/industries/agriculture" },
    { name: "Construction", link: "/industries/construction" },
    { name: "Mining & Quarrying", link: "/industries/mining-quarrying" },
    { name: "Forest Conservation", link: "/industries/forest-conservation" }
  ];

  return (
    <div className="min-h-screen">
      <Navbar />
      <main className="pt-24 pb-16">
        <div className="section-container">
          <div className="text-center mb-12 animate-fade-in">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Industries</h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Serving diverse sectors with specialized UAV solutions
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {industries.map((industry, index) => (
              <Link
                key={index}
                to={industry.link}
                className="glass-card group hover:scale-105 transition-all duration-300 text-center"
              >
                <h3 className="text-xl font-bold">{industry.name}</h3>
              </Link>
            ))}
          </div>
        </div>
      </main>
      <Footer />
    </div>
  );
};

export default Industries;
